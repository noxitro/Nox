...
namespace
{
	...
	[[nodiscard]] nox::hid::KeyCode KeyCodeFromScanCode(const nox::os::RawKeyboardInputEvent& event)noexcept
	{
	...
	void* const user_data)noexcept
{
	...
	if (manager->producer_resync_pending_ == true)
	{
		if (manager->TryPushReleaseAll(QueuedEventType::Resync) == false)
		{
			return;
		}
		manager->producer_resync_pending_ = false;
	}

	if (event.type == nox::os::RawKeyboardInputType::FocusLost)
	{
		if (manager->TryPushReleaseAll(QueuedEventType::ReleaseAll) == false)
		{
			manager->producer_resync_pending_ = true;
		}
		return;
	}
	...
	if (manager->TryPush({ QueuedEventType::Key, key_code, is_down }) == false)
	{
		manager->producer_resync_pending_ = true;
		return;
	}
...
{
	const nox::hid::KeyCode key_code = KeyCodeFromScanCode(event);
	if (key_code != nox::hid::KeyCode::Unknown)
	{
		return key_code;
	}
	return KeyCodeFromVirtualKey(event.virtual_key);
}

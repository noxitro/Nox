...
namespace nox::os
{
	...
	namespace
	{
		struct WindowMessageHookEntry
		{
			nox::os::WindowMessageHook hook;
			void* user_data;
		};

		/// @brief 登録済みのフック。ウィンドウメッセージを処理するスレッドだけが触る
		constinit std::array<WindowMessageHookEntry, nox::os::kMaxWindowMessageHooks> window_message_hooks_{};
		constinit nox::uint32 window_message_hook_count_ = 0u;
	}
}

bool nox::os::AddWindowMessageHook(const nox::os::WindowMessageHook hook, void* const user_data)noexcept
{
	if (hook == nullptr)
	{
		NOX_ASSERT(false, u8"ウィンドウメッセージのフックが null です");
		return false;
	}

	for (nox::uint32 hook_index = 0u; hook_index < window_message_hook_count_; ++hook_index)
	{
		const WindowMessageHookEntry& entry = window_message_hooks_[hook_index];
		if (entry.hook == hook && entry.user_data == user_data)
		{
			NOX_ASSERT(false, u8"同じウィンドウメッセージのフックが登録済みです");
			return false;
		}
	}

	if (window_message_hook_count_ >= nox::os::kMaxWindowMessageHooks)
	{
		NOX_ASSERT(false, u8"ウィンドウメッセージのフックが上限に達しました");
		return false;
	}

	window_message_hooks_[window_message_hook_count_] = WindowMessageHookEntry{ hook, user_data };
	++window_message_hook_count_;
	return true;
}

void nox::os::RemoveWindowMessageHook(const nox::os::WindowMessageHook hook, void* const user_data)noexcept
{
	for (nox::uint32 hook_index = 0u; hook_index < window_message_hook_count_; ++hook_index)
	{
		const WindowMessageHookEntry& entry = window_message_hooks_[hook_index];
		...
		//	呼び出し順が登録順のままになるよう詰める
		for (nox::uint32 next_index = hook_index + 1u; next_index < window_message_hook_count_; ++next_index)
		{
			window_message_hooks_[next_index - 1u] = window_message_hooks_[next_index];
		}
		--window_message_hook_count_;
		window_message_hooks_[window_message_hook_count_] = WindowMessageHookEntry{};
		return;
	}
}
...
::LRESULT CALLBACK nox::os::Window::CallbackWindow(const ::HWND hWnd, const ::UINT message, const ::WPARAM wParam, ::LPARAM lParam)
{
	...
	if (window_message_hook_count_ != 0u)
	{
		const nox::os::WindowMessage hook_message
		{
			.window_handle = hWnd,
			.message = static_cast<nox::uint32>(message),
			.wparam = static_cast<nox::uint64>(wParam),
			.lparam = static_cast<nox::int64>(lParam)
		};
		for (nox::uint32 hook_index = 0u; hook_index < window_message_hook_count_; ++hook_index)
		{
			const WindowMessageHookEntry& entry = window_message_hooks_[hook_index];
			entry.hook(hook_message, entry.user_data);
		}
	}

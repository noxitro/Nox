...
混同しやすいので注意: 隣の `runtime/core/test_support/` は **このプロジェクトのものではない**。
あちらは `core.vcxproj` がビルドする `core` 内部のコードで、

- 旧 `NOX_ASSERT` ベースのセルフテスト本体（`core_self_test.cpp` から gtest 経由で呼ぶ）
- リフレクション生成器に見せるテスト用型（`test_types.h` 系）

が入っている。以前は `core/test/` という名前だったが、GoogleTest プロジェクトを
ここへ移すときに役割が分かるよう改名した。
...
| `core_self_test.cpp` | `core/test_support/*.cpp` にある `NOX_ASSERT` ベースのセルフテストを gtest のケースとして走らせる |
...
エスケープハッチ）を使っている。おかげでテスト専用の型を `core/test_support/test_types.h` へ
...
`runtime/core/test_support/reflection_variable_test_types.h` にある。

リフレクション生成器は解析の起点 (`reflection_generated/reflect.cpp`) から
辿れる型しか見ないため、テスト専用の型もそこから見える場所に置く必要がある。
既に同じ目的で `core/test_support/test_types.h` という集約ヘッダがあるので、それに乗せている
